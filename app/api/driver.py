...
def register_driver(driver_data: DriverCreate, db: Session = Depends(get_db), current_user: User = Depends(verify_token_and_login)):
    return driver_service.create_driver(driver_data, current_user.id, db)

@router.get("/", response_model=list[DriverResponse])
def get_available_drivers(db: Session = Depends(get_db), current_user: User = Depends(verify_token_and_login)):
    return driver_service.get_all_drivers(db)

@router.get("/{driver_id}", response_model=DriverResponse)
def get_driver(driver_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(verify_token_and_login)):
...
def update_driver(driver_id: UUID, updates: DriverUpdate, db: Session = Depends(get_db), current_user: User = Depends(verify_token_and_login)):
    driver = driver_service.get_driver_by_id(driver_id, db)
    if not driver:
        raise HTTPException(status_code=404, detail="Driver not found")
...
def delete_driver(driver_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(verify_token_and_login)):
    driver = driver_service.get_driver_by_id(driver_id, db)
    if not driver:
        raise HTTPException(status_code=404, detail="Driver not found")
